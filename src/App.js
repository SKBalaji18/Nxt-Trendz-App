...
class App extends Component {
    ...
    if (existingProduct) {
      this.setState(prevState => ({
        cartList: prevState.cartList.map(eachItem => {
          if (existingProduct.id === eachItem.id) {
            const updatedQuatity = eachItem.quantity + product.quantity
            return {...eachItem, quantity: updatedQuatity}
          }
          return eachItem
        }),
      }))
  ...
  incrementCartItemQuantity = id => {
    this.setState(prevState => ({
      cartList: prevState.cartList.map(eachItem => {
        if (id === eachItem.id) {
          const updatedQuatity = eachItem.quantity + 1
          return {...eachItem, quantity: updatedQuatity}
        }
        return eachItem
      }),
